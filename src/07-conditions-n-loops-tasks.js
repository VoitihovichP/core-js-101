...
function isInsideCircle(circle, point) {
  const cords = Math.sqrt((circle.center.x - point.x) ** 2 + (circle.center.y - point.y) ** 2);
  return cords < circle.radius;
}
...
function reverseString(str) {
  ...
  const resultArr = wordsArr.map((el) => {
    const elStr = el.split('').reverse().join('');
    return elStr;
  });
...
function getDigitalRoot(num) {
  const numArr = [...num.toString()].map((el) => Number(el));
  const sum = numArr.reduce((a, b) => a + b);
  if (sum > 9) {
    const subNumArr = [...sum.toString()].map((el) => Number(el));
    const subSum = subNumArr.reduce((a, b) => a + b);
    return subSum;
  }
  return sum;
}
